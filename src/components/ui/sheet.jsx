...
const sheetVariants = cva(
    // use explicit white background, subtle shadow, and smooth transitions
...
const SheetContent = React.forwardRef(({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
        ...
        <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
            {children}
            {/* close button: light gray background with subtle hover, icon dark */}
            <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm bg-gray-100 opacity-90 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:pointer-events-none">
